...
def calculate_edge_cycles_degree(G, cycles):
    edge_cycles_degree = {}
    for edge in G.es: 
        edge_cycles_degree[edge.index] = 0
        for sub_cycles in cycles:
            if edge.source in sub_cycles and edge.target in sub_cycles:
                edge_cycles_degree[edge.index] += 1
    return edge_cycles_degree
...
def main():
    ...
    for i, graph_name in enumerate(Graphs):
        ...
        for j in range(4):
            ...
            edge_cycles_degree = calculate_edge_cycles_degree(G, cycles)
            my_dict = collections.Counter(edge_cycles_degree.values())  
        ...
        ax.set_xlabel("Cycle degree of edge")
    ...
    plt.savefig('figure/edge_cycles_degree.eps', dpi=600, bbox_inches='tight')
    plt.savefig('figure/edge_cycles_degree.svg', dpi=600, bbox_inches='tight')
    plt.show()
