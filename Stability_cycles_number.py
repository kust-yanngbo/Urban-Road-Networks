...
for i in range(6):  
    ...
    # 计算各个环的数量G:\城市道路数据分析\鲁棒性分析
    file_path = r'data\\Robustness\\' + Graph[i] +'_2_' + attack[2] + '.csv'
    ...
    ax.set_xlabel("$p$") 
    ax.set_ylabel("$N^{o}$", rotation=0, labelpad=15)
    ax.set_xticks(np.arange(0, 1.2, 0.2))

fig.legend(labels1, loc='lower center', bbox_to_anchor=(0.5, -0.07), ncol=3)
fig.tight_layout()  # 调整子图布局，使之不会重叠 
plt.savefig('figure/Stability_cycle_number.eps', dpi=600, bbox_inches='tight')
plt.savefig('figure/Stability_cycle_number.svg', dpi=600, bbox_inches='tight')
plt.show()