...
for i in range(6):  
    for j in range(4):
        ...
        # graph_file =  Graph[i] +'_'+ str(j+1)+ '_dual_1'
        # 计算度分布
        file_path = path + Graph[i] +'_'+ str(j+1) + '.graphml'
        # file_path = '城市道路数据分析/对偶图_共享一个节点/' + Graph[i] +'_'+ str(j+1) + '_dual_1.graphml'
...
# 将结果保存为 CSV 文件
df.to_csv(path+'city_network_metrics_igraph.csv', index=False)
# df.to_csv('城市道路数据分析/网络指标/dual_network_metrics_igraph.csv', index=False)
