...
# 常量
DATA_PATH = 'data/'
GRAPH_FILE_FORMAT = '{}_{}_dual_1.graphml'
...
Markers = ['o', '^', 'v', 'p']
Labels = ['(a) First layer', '(b) Second layer', '(c) Third layer', '(d) Fourth layer']
...
for i in range(6):  
    ...
    for j in range(4):
        graph_path = DATA_PATH+'DualNetwork_Node1/'+ GRAPH_FILE_FORMAT.format(Graphs[i], j + 1)
        ...
        # G.add_vertices(G_directed.vcount())
        # G.add_edges(G_directed.get_edgelist())
        # G.es['weight'] = G_directed.es['weight']
        
        # my_dict = collections.Counter(G.degree()) 
        my_dict = collections.Counter(G.es['weight'])
...
plt.savefig('figure/Dual_graph_weight.eps', dpi=600, bbox_inches='tight')
plt.savefig('figure/Dual_graph_weight.svg', dpi=600, bbox_inches='tight')
...
df['City'] = Graphs
df.to_csv('data/NetworkMetrics/dual_grap_weight_powerlaw_results.csv', index=False)
